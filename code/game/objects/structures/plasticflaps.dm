...
/obj/structure/plasticflaps/CanAllowThrough(atom/movable/mover, turf/T)
	...
	if(istype(mover, /obj/structure/bed) && (B.has_buckled_mobs() || B.density))//if it's a bed/chair and is dense or someone is buckled, it will not pass
		return FALSE

	if(istype(mover, /obj/structure/closet/cardboard))
		var/obj/structure/closet/cardboard/C = mover
		if(C.move_delay)
			return FALSE
	...
	if(ismecha(mover))
		...
		if(istype(living_mover.buckled, /mob/living/simple_animal/bot/mulebot)) // mulebot passenger gets a free pass.
			return TRUE
		if((living_mover.mobility_flags & MOBILITY_STAND) && living_mover.mob_size != MOB_SIZE_TINY && !(HAS_TRAIT(living_mover, TRAIT_VENTCRAWLER_ALWAYS) || HAS_TRAIT(living_mover, TRAIT_VENTCRAWLER_NUDE)))
			return FALSE //If your not laying down, or a ventcrawler or a small creature, no pass.
